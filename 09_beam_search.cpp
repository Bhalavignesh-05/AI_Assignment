...
PathResult beam_search(const Graph& g, const Heuristics& h, char start, char goal, int beam_width) {
...
int main() {
    int beam_width = 2;
    PathResult result = beam_search(graph, heuristics, START_NODE, GOAL_NODE, beam_width);
    print_result("Beam Search (Width=2)", result);
    return 0;
}